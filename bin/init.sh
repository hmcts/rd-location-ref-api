...
declare -a files_with_slug=(build.gradle docker-compose.yml Dockerfile README.md ./infrastructure/main.tf ./src/main/java/uk/gov/hmcts/reform/demo/controllers/RootController.java charts/rd-location-ref-api/Chart.yaml)
...
# Replace image repo
for i in "charts/rd-location-ref-api/values.yaml"
do
  perl -i -pe "s/rpe/$product_name/g" ${i}
  perl -i -pe "s/location-ref-api.jar/$component_name/g" ${i}
done

#update maintainer name
for i in "charts/rd-location-ref-api/Chart.yaml"
do
  perl -i -pe "s/rpe/$product_name/g" ${i}
done
...
do
  perl -i -pe "s/location-ref-api.jar/$slug/g" ${i}
done
...
git mv charts/rd-location-ref-api charts/${slug}
